...
public class Map {
	...
	public void setUnitData()
	{
		...
		for (bwapi.Unit unit :  Globals.game.getAllUnits())
		{
			
			if (!unit.getType().isBuilding())
			{
				addUnit(unit); // Do we need to create a "hard copy"? The original code implied that we did
			}
		}
	}
	...
	public void addUnit(Unit unit)
	{
		if (unit.getType().isBuilding())
		{
			int tx = unit.getX() / Constants.TILE_SIZE;
			int ty = unit.getY() / Constants.TILE_SIZE;
		...
		{
			int startX = (unit.getX() - unit.getType().dimensionRight()) / Constants.TILE_SIZE;
			int endX   = (unit.getX() + unit.getType().dimensionRight() + Constants.TILE_SIZE - 1) / Constants.TILE_SIZE; // Division - round up
			int startY = (unit.getY() - unit.getType().dimensionUp()) / Constants.TILE_SIZE;
			int endY   = (unit.getY() + unit.getType().dimensionDown() + Constants.TILE_SIZE - 1) / Constants.TILE_SIZE;
